<script lang="ts">import { AsciiEffect } from 'three/examples/jsm/Addons.js';
...
const { autoRender: threlteAutoRender, camera: defaultCamera, renderStage, renderer, scene: defaultScene, size } = useThrelte();
...
observe.pre(() => [autoRender], ([autoRender]) => {
    if (autoRender) {
        start();
    }
    else {
