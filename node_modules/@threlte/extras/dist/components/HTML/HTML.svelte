...
import { Vector3, Group, Mesh, PerspectiveCamera, Object3D, DoubleSide, Matrix4, OrthographicCamera, Raycaster } from 'three';
import { defaultCalculatePosition, epsilon, getCameraCSSMatrix, getObjectCSSMatrix, isObjectBehindCamera, isObjectVisible, objectScale, objectZIndex, getViewportFactor } from './utils';
import { logVertex, logFragment, spriteVertex } from './shaders';
let { autoRender = true, eps = 0.001, center = false, fullscreen = false, portal, distanceFactor, sprite = false, transform = false, occlude = false, castShadow, receiveShadow, material, geometry, zIndexRange = [16777271, 0], calculatePosition = defaultCalculatePosition, as = 'div', wrapperClass, pointerEvents = 'auto', ref = $bindable(), visible = $bindable(), style, children, ...props } = $props();
visible = true;
